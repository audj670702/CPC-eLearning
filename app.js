...
function bindUI() {
  ...
  audioBtn?.addEventListener('click', async () => { if (!video) return; const enableAudio = video.muted; video.muted = !enableAudio; video.defaultMuted = !enableAudio; const frame = document.getElementById('cpcTvYoutubeFrame'); if (frame?.contentWindow) frame.contentWindow.postMessage(JSON.stringify({ event: 'command', func: enableAudio ? 'unMute' : 'mute', args: [] }), 'https://www.youtube.com'); try { await video.play(); } catch (_) {} audioBtn.setAttribute('aria-pressed', String(enableAudio)); audioBtn.textContent = enableAudio ? '🔊' : '🔇'; audioBtn.setAttribute('aria-label', enableAudio ? 'Silenciar' : 'Activar sonido'); audioBtn.title = enableAudio ? 'Silenciar' : 'Activar sonido'; });
