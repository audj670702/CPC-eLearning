...
const MNS_FRAME_URL = 'mns-frontend.html?v=0.5.0';
...
function ensureStyles(){
  ...
  style.textContent=`
    .cpc-mns-overlay{position:fixed;inset:0;z-index:99999;background:rgba(11,28,47,.46);display:flex;align-items:stretch;justify-content:center}
    ...
    @media(min-width:760px){.cpc-mns-overlay{padding:24px}.cpc-mns-panel{max-width:540px;height:calc(100dvh - 48px);border-radius:22px;box-shadow:0 20px 70px rgba(6,31,57,.26)}}
  `;
  document.head.appendChild(style);
}

function getFrame(){return document.querySelector('#cpcMnsOverlay iframe');}
function closeMns(){document.getElementById('cpcMnsOverlay')?.remove();document.body.classList.remove('cpc-mns-open');}
...
function openMns(){
  if(!readTokens()?.accessToken?.value){document.querySelector('.session-btn')?.click();return;}
  ensureStyles();closeMns();
  const overlay=document.createElement('div');
  overlay.id='cpcMnsOverlay';overlay.className='cpc-mns-overlay';
  overlay.innerHTML=`<div class="cpc-mns-panel" role="dialog" aria-modal="true" aria-label="Mensajería"><iframe class="cpc-mns-frame" src="${MNS_FRAME_URL}" title="Mensajería SCaD MNS"></iframe></div>`;
  overlay.addEventListener('click',e=>{if(e.target===overlay)closeMns();});
  document.body.appendChild(overlay);document.body.classList.add('cpc-mns-open');
}

function bindMnsCard(){
  const card=findMessagingCard();
  if(!card||card.dataset.cpcMnsBound==='true')return false;
  card.dataset.cpcMnsBound='true';card.addEventListener('click',openMns);return true;
}
...
window.addEventListener('message',async event=>{
  ...
  if(message.type==='CLOSE'){closeMns();return;}
  if(message.type==='READY'){
    frame.contentWindow.postMessage({channel:CHANNEL,type:'CONTEXT',payload:MNS_CONTEXT},window.location.origin);
    return;
  }
  ...
  try{
    ...
    frame.contentWindow.postMessage({channel:CHANNEL,id:message.id,ok:true,data},window.location.origin);
  }catch(error){
    frame.contentWindow.postMessage({channel:CHANNEL,id:message.id,ok:false,error:error?.message||'Error MNS'},window.location.origin);
  }
});

const observer=new MutationObserver(()=>{if(bindMnsCard())observer.disconnect();});
if(!bindMnsCard())observer.observe(document.documentElement,{childList:true,subtree:true});
